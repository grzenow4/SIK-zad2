...
GameRoom::GameRoom(boost::asio::io_context &io_context, ServerParameters server_params) :
...
uint32_t GameRoom::random() {
    uint32_t res = (uint32_t) ((uint64_t) _next_random_num * Q % MOD);
    _next_random_num = res;
    return res;
}
...
void GameRoom::resolve_join(std::shared_ptr<GameSession> participant, Player player) {
    if (!_game_in_progress) {
        ...
        for (auto participant: _players) {
            participant->send_accepted_player(_next_id, player);
        }
        _next_id++;
...
void GameRoom::explode_bombs() {
    for (auto el: _game_status.bombs) {
        ...
        if (bomb.timer == 0) {
            ...
            for (int i = bomb.position.x; i >= 0 && i >= bomb.position.x - _server_params.get_explosion_radius(); i--) {
                Position p{(uint16_t) i, bomb.position.y};
                for (auto it = _game_status.player_positions.begin(); it != _game_status.player_positions.end(); it++) {
                    if (it->second == p) {
            ...
            for (int i = bomb.position.x; i < _server_params.get_size_x() && i <= bomb.position.x + _server_params.get_explosion_radius(); i++) {
                Position p{(uint16_t) i, bomb.position.y};
                for (auto it = _game_status.player_positions.begin(); it != _game_status.player_positions.end(); it++) {
                    if (it->second == p) {
                        robots_destroyed.emplace_back(it->first);
            ...
            for (int i = bomb.position.y; i >= 0 && i >= bomb.position.y - _server_params.get_explosion_radius(); i--) {
            ...
            for (int i = bomb.position.y; i < _server_params.get_size_y() && i <= bomb.position.x + _server_params.get_explosion_radius(); i++) {
                Position p{bomb.position.x, (uint16_t) i};
                for (auto it = _game_status.player_positions.begin(); it != _game_status.player_positions.end(); it++) {
                    if (it->second == p) {
                        robots_destroyed.emplace_back(it->first);
...
void GameRoom::handle_turn() {
    _timer.expires_from_now(boost::asio::chrono::milliseconds(_server_params.get_turn_duration()));
        ...
        if (ec) {
            ...
            for (auto pair: _moves) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    resolve_move(pair.first, (Direction) pair.second.second);
                    break;
                }
            }
            _moves.clear();
            ...
            for (auto participant: _players) {
                participant->send_turn(_game_status.turn,
                                       _game_status.turns[_game_status.turn]);
            }
